...
- `src/visual-compare.js`: `openprd visual-compare` 的图片合成工具，负责读取效果图和实现截图、缩放面板、添加“效果图 / 实现截图”标签，并输出 JPG / PNG / WebP 视觉评审图。
...
- `.openprd/harness/visual-reviews/`: 界面视觉对比图归档，默认存放 `openprd visual-compare` 输出的 JPG。
