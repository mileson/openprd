...
test('setup enables Codex hooks while preserving user hook groups', async () => {
  ...
  try {
    ...
    assert.ok(generatedAgents.includes('Image 2 是工具路径，不是审美豁免'));
    assert.ok(generatedAgents.includes('默认紫白/蓝紫渐变、通用字体、白底卡片堆叠和无语境装饰'));
    ...
    assert.ok(generatedAgents.includes('动手前要有一句审美意图和记忆点'));
    assert.ok(generatedAgents.includes('visual evidence 同时检查气质、层级、字体/色彩/表面角色和记忆点'));
    ...
    assert.ok(generatedHook.includes('Image 2 是工具路径，不是审美豁免'));
    assert.ok(generatedHook.includes('审美意图和记忆点'));
    ...
    assert.ok(commandCatalog.includes('审美意图和记忆点'));
    assert.ok(commandCatalog.includes('不能替代 `visual-compare` 或证据板'));
    assert.ok(commandCatalog.includes('对齐辅助线证据板'));
    assert.ok(commandCatalog.includes('内部居中证据板'));
    assert.ok(commandCatalog.includes('centering-board'));
    assert.ok(commandCatalog.includes('视觉重心'));
    assert.ok(commandCatalog.includes('相同文案类型/相同组件槽位'));
    assert.ok(commandCatalog.includes('内容槽位'));
    ...
    assert.ok(generatedRouterSkill.includes('解释型 SVG'));
    assert.ok(generatedRouterSkill.includes('不要把它误升级成正式评审图'));
    ...
    assert.ok(generatedIntakeSkill.includes('解释型 SVG'));
    const generatedDiagramSkill = await fs.readFile(path.join(project, '.codex', 'skills', 'openprd-diagram-review', 'SKILL.md'), 'utf8');
    assert.ok(generatedDiagramSkill.includes('explanation-svg'));
    assert.ok(generatedDiagramSkill.includes('轻量解释图'));
    const generatedExplanationSvgPatterns = await fs.readFile(path.join(project, '.codex', 'skills', 'openprd-diagram-review', 'references', 'explanation-svg-patterns.md'), 'utf8');
    assert.ok(generatedExplanationSvgPatterns.includes('State Transition'));
    assert.ok(generatedExplanationSvgPatterns.includes('Boundary Map'));
    ...
    assert.ok(generatedCommandCatalog.includes('对齐辅助线证据板'));
    assert.ok(generatedCommandCatalog.includes('内部居中证据板'));
    assert.ok(generatedCommandCatalog.includes('主体外接框中心偏移'));
    assert.ok(generatedCommandCatalog.includes('相同组件槽位'));
    assert.ok(generatedCommandCatalog.includes('内部内容槽位'));
    assert.ok(generatedCommandCatalog.includes('只量外框'));
    ...
    assert.ok(generatedQualitySkill.includes('对齐辅助线证据板'));
    assert.ok(generatedQualitySkill.includes('内部内容槽位'));
    assert.ok(generatedQualitySkill.includes('只量了外框'));
    assert.ok(generatedQualitySkill.includes('审美主张和记忆点'));
    assert.ok(generatedQualitySkill.includes('不只查有无图片'));
    ...
    assert.ok(generatedSharedSkill.includes('Image 2 是工具路径，不是审美豁免'));
    assert.ok(generatedSharedSkill.includes('视觉证据不仅检查位置'));
    ...
    assert.ok(generatedSharedSkill.includes('图解优先'));
    assert.ok(generatedSharedSkill.includes('不替代 `review.html`'));
    assert.ok(generatedSharedSkill.includes('高置信应可成长'));
    assert.ok(generatedSharedSkill.includes('openprd update .'));
    assert.ok(generatedSharedSkill.includes('已有确认参考图时对比“效果图 / 实现截图”'));
    assert.ok(generatedSharedSkill.includes('用户可见文档、进度日志、proposal'));
    assert.ok(generatedSharedSkill.includes('spec 和 tasks 跟随用户当前主语言'));
    ...
    assert.ok(generatedHarnessSkill.includes('alignment-board'));
    assert.ok(generatedHarnessSkill.includes('相同组件槽位'));
    ...
    assert.ok(generatedCodexHook.includes('Image 2 是工具路径，不是审美豁免'));
    assert.ok(generatedCodexHook.includes('没有品牌或参考图依据时，不要默认紫白/蓝紫渐变'));
    ...
    assert.ok(generatedCodexHook.includes('至少产出一份 visual-compare / focus-board / parallel-board / verification-board / alignment-board / centering-board 证据图'));
    assert.ok(generatedCodexHook.includes('截图实测证据板'));
    assert.ok(generatedCodexHook.includes('对齐辅助线证据板'));
    assert.ok(generatedCodexHook.includes('内部居中证据板'));
    assert.ok(generatedCodexHook.includes('视觉重心偏移'));
    assert.ok(generatedCodexHook.includes('内容槽位'));
    assert.ok(generatedCodexHook.includes('只量外框'));
    ...
    assert.ok(generatedFrontendSkill.includes('相同文案类型/相同组件槽位'));
    assert.ok(generatedFrontendSkill.includes('alignment-board'));
    assert.ok(generatedFrontendSkill.includes('centering-board'));
    assert.ok(generatedFrontendSkill.includes('黄色视觉重心点'));
    assert.ok(generatedFrontendSkill.includes('标题、副标题、描述、标签、状态、价格、按钮'));
    ...
    assert.ok(generatedVisualCommand.includes('mode=alignment-board'));
    assert.ok(generatedVisualCommand.includes('mode=centering-board'));
    assert.ok(generatedVisualCommand.includes('visual-centroid offset'));
    assert.ok(generatedVisualCommand.includes('Internal content slots'));
