...
![OpenPrd 场景总览](./docs/assets/openprd-scenario-overview.png)
...
| 场景 | OpenPrd 帮你完成什么 | 主要产物 |
|------|----------------------|----------|
| 新产品想法 | 先把问题、范围、已确认事实和待确认问题说清楚，再生成可评审 PRD。 | `clarify`、`capture`、`synthesize`、`review.html` |
| 已有项目接入 | 从现有行为里梳理证据、覆盖缺口和标准化问题，再沉淀成结构化 change。 | `discovery`、`standards`、`change`、`tasks` |
| 流程或界面确认 | 在进入实现或验收前，用架构图、产品流程图或左右对比图确认理解是否一致。 | `diagram`、`visual-compare`、左右对比 JPG |
| Agent 长程执行 | 把已确认的工作拆成按依赖排序的小任务，每次新会话只处理一个任务。 | `loop`、任务提示词、进度日志、验证报告 |
| 交付前检查 | 在提交、发布或交接前检查标准化、回归证据、成本与滥用风险、工作区健康度。 | `quality`、`run --verify`、`doctor`、`handoff` |
